...
let bvmL1CrossDomainMessenger: MockBvmL1CrossDomainMessenger;
let bvmL2CrossDomainMessenger: MockBvmL2CrossDomainMessenger;
let bridgeExecutor: MantleBridgeExecutor;
...
const MANTLE_GAS_LIMIT = 15000000;

const encodeSimpleActionsSet = (
  bridgeExecutor: MantleBridgeExecutor,
  target: string,
  fn: string,
  params: any[]
) => {
...
describe('MantleBridgeExecutor', async function () {
  let snapId;
  ...
  before(async () => {
    ...
    // Mocking Mantle bridge messenger
    bvmL1CrossDomainMessenger = await new MockBvmL1CrossDomainMessenger__factory(user).deploy();
    bvmL2CrossDomainMessenger = await new MockBvmL2CrossDomainMessenger__factory(user).deploy();
    await bvmL1CrossDomainMessenger.setL2Messenger(bvmL2CrossDomainMessenger.address);
    await bvmL2CrossDomainMessenger.setL1Messenger(bvmL2CrossDomainMessenger.address);

    bridgeExecutor = await new MantleBridgeExecutor__factory(user).deploy(
      bvmL2CrossDomainMessenger.address,
  ...
  it('Check initial parameters', async () => {
    ...
    // Mantle Bridge Executor parameters
    expect(await bridgeExecutor.getEthereumGovernanceExecutor()).to.be.equal(
      ethereumGovernanceExecutor.address
    );
    expect(await bridgeExecutor.BVM_L2_CROSS_DOMAIN_MESSENGER()).to.be.equal(
      bvmL2CrossDomainMessenger.address
    );
  });
  ...
  context('Ethereum Governance Executor queues an actions sets', () => {
    ...
    it('Tries to queue and actions set via Mantle L2 Messenger without being the Ethereum Governance Executor (revert expected)', async () => {
      ...
      await expect(
        bvmL1CrossDomainMessenger
          .connect(user)
          .sendMessage(bridgeExecutor.address, encodedData, MANTLE_GAS_LIMIT, {
            gasLimit: 12000000,
          })
    ...
    it('Queue and execute an actions set to set a message in Greeter', async () => {
      ...
      const tx = await bvmL1CrossDomainMessenger
  ...
  context('Update parameters', () => {
    ...
    it('Tries to update the delays with wrong configuration (revert expected)', async () => {
      ...
      for (const wrongConfig of wrongConfigs) {
        expect(
          await bvmL1CrossDomainMessenger
            .connect(ethereumGovernanceExecutor)
            .sendMessage(bridgeExecutor.address, wrongConfig.encodedData, MANTLE_GAS_LIMIT, {
              gasLimit: 12000000,
            })
        );
      }
